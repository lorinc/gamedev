...
| D033 | 2026-09-25 | p4 | active | Stops get the swipe cue too (user's call): when a run stops on its own, the disc appears in the run's direction showing what the next step would do (arrow, stairs or pickaxe), with a "?" badge. It's white, never red, because it isn't a refusal. |
