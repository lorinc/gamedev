// b2 · Darkness (forked from b1.7, D049): wires sim + input + render + juice + dev panel. Fixed 60 Hz sim, render interpolates.
// The rules come from a ruleset: rules/b2.1.json, or with ?rules=lab the one the Rule Lab (v4.html)
...
const BUILD = 'b2-dev' // the live build; npm run freeze stamps the build id into frozen copies
const STORE = 'b2-tunables-changed' // only values tuned away from DEFAULTS
...
async function loadRuleset() {
  if (new URLSearchParams(location.search).get('rules') === 'lab') {
    ...
    alert('No Rule Lab ruleset saved in this browser: playing rules/b2.1.json')
  }
  return migrate(await (await fetch('rules/b2.1.json', { cache: 'no-cache' })).json())
}
...
function start(ruleset, table) {
  ...
  Object.assign(window, { b2: { game, tunables, ruleset } })
