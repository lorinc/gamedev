...
// The rules come from a ruleset: rules/b3.3.json, or with ?rules=lab the one the Rule Lab (v4.html)
...
const BUILD = 'b3.3' // the live build; npm run freeze stamps the build id into frozen copies
...
async function loadRuleset() {
  if (new URLSearchParams(location.search).get('rules') === 'lab') {
    ...
    alert('No Rule Lab ruleset saved in this browser: playing rules/b3.3.json')
  }
  return migrate(await (await fetch('rules/b3.3.json', { cache: 'no-cache' })).json())
}
...
function start(ruleset, table) {
  ...
  function readout() {
    ...
    return [
      ...
      `bugs: ${game.bugs.filter((b) => b.kind === 'wild').length} wild, fed ${game.fed} · bar ${game.bar.length} · placed ${game.bugs.filter((b) => b.kind === 'placed').length}`,
